...
....
$ oc new-project bluegreen-userX --display-name="Blue Green Project" --description="Blue Green Project"
....
...
`$ oc expose service blue --hostname=bluegreen.userX.{routing-suffix} --name=bluegreen`
...
You can route traffic to both services. This is also known as "cannary in a coalmine".
...
Next, select `Split traffic across multiple services`

...
*Note:* Please replace *userX* with the username assigned to you in
the commands below.

....
$ oc new-project mycliproject-userX --description="My CLI Project" --display-name="CLI Project"
....
...
In project CLI Project (mycliproject-UserX)
...
`apps.workshop.osecloud.com`. Use {routing-suffix} as your domain name
instead.

....
$ oc expose service welcome --hostname=welcome.UserName.apps.workshop.open.redhat.com

NAME      HOST/PORT                     PATH      SERVICE   LABELS
welcome   welcome.UserName.apps.workshop.open.redhat.com             welcome
....

*Step 4: Try your application*

Access the application: Now access the application using curl (looking
for 200 status code) or from the browser and see the result. Make sure you use
the `http` protocol instead of `https`.
...
NAME      HOST/PORT                     PATH      SERVICE   LABELS
welcome   welcome.apps.workshop.open.redhat.com             welcome

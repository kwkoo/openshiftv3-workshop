...
*Note*: This section assumes that you are using Github as a repository. If you
do not have a Github account, you can use the Gogs server at {gogs-url}. Login
with the username `userX` and the password `gogs`.

Navigate back to the `configuration` tab for the pipeline as explained
in Step 3. Copy the value for `Github webhook` url (copy the `Generic webhook`
url if using Gogs).
Since development-userX uses the git repo from
https://github.com/RedHatWorkshops/welcome-php), you will need to fork this
repo and update the development configure to use the forked repo.

Based on what you learned in the past, go to your github repository that
you cloned and set up a webhook pointing to this URL.

*Tips*

* Navigate to `Settings` -> `Webhooks` on your project in github
* Set the `Payload URL` to the `Github Webhook` URL noted above

...
binaries for Mac, Windows, and Linux are available for download from:
{oc-download-url}
...
2.  *Windows:*
+
2.1. Install git and git bash for Windows from
https://git-scm.com/download/win. During installation you need to
specify in the selection to integrate with the command prompt.
+
2.2. Use oc.exe to open an OpenShift shell. If you getting error from
...
command prompt)
+
2.3. Download and install Notepad++ and install the JSON plugin or use
http://jsonlint.com/ to edit and validate JSON.
+
http://ammonsonline.com/formatted-json-in-notepad/
+
2.4. Configure your default editor to be Notepad++
...
Use the following for the login:
...
Some of the labs require the execution of `curl`. If you are performing
the labs from a Windows machine, you can download curl from:

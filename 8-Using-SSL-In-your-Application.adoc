...
will be using the *myjbossapp-userX* that you created in the previous
labs. Make sure you are switched to that project by using the *oc
project* command. *Remember* to substitute `userX`.

....
$ oc project myjbossapp-userX
....
...
spec:
  host: ks-myjbossapp-userX.apps.openshiftworkshop.com
  port:
    targetPort: 8080-tcp
  to:
    kind: Service

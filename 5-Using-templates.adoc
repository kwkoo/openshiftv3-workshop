...
Note: If you are on Windows and not using the bash shell, the curl command
above will not work. Find out the hostname of the pricelist route and
execute `curl -k http://PRICELIST_HOSTNAME/create_database.php`.
